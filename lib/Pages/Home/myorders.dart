...
class _OrdersPageState extends State<OrdersPage> {
  ...
  Future<void> _reload() async {
    setState(() => _future = _svc.getMyOrders());
    await _future;
  }
  ...
  Future<void> _refreshStatus() async {
    try {
      await _reload();
      if (!mounted) return;
      ToastHelper.showCustomToast(context, 'Status refreshed', isSuccess: true, errorMessage: '');
    } catch (e) {
      if (!mounted) return;
      ToastHelper.showCustomToast(context, 'Refresh failed', isSuccess: false, errorMessage: e.toString());
    }
  }
    ...
    return Row(
      children: [
        ...
        if (trailing != null) ...[
          ...
          Text(
            trailing,
            style: const TextStyle(color: Color(0xFF6B778C), fontWeight: FontWeight.w600),
          ),
        ],
      ],
    );
  ...
  Widget build(BuildContext context) {
    final titleStyle = Theme.of(context)
        .textTheme
        .headlineSmall
        ?.copyWith(fontWeight: FontWeight.w800, color: const Color(0xFF222222));
